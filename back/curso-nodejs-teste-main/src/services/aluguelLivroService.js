...
class AluguelLivroService {
  ...
  async calcularDataDevolucao(dataAlugado, numeroDiasAlugado) {
    try {
      if (!dataAlugado) {
        throw new Error("Dia de início do aluguel não especificado");
      } else if (numeroDiasAlugado <= 0) {
        throw new Error("Quantos dias alugados não especificado");
      }

      const data = new Date(dataAlugado);
      data.setDate(data.getDate() + numeroDiasAlugado);
      return data;
    } catch (err) {
      throw new Error(err.message);
    }
